...
{
	LOG_DIRECTORY=$1
	ls -t ${LOG_DIRECTORY} | head -n 1 | xargs -I {} head -n 1 ${LOG_DIRECTORY}/{}
}

function restore_serverlog_tests()
{
	TEST_NAME=$1
	LOG_DIRECTORY=$2
	OPTION=${3:-""}

	check_log ${LOG_DIRECTORY} > ${TEST_BASE}/${TEST_NAME}-before.out
	pg_rman backup -B ${BACKUP_PATH} -b full -p ${TEST_PGPORT} -d postgres --quiet ${OPTION};echo $?
	pg_rman validate -B ${BACKUP_PATH} --quiet
	#pg_rman show detail -B ${BACKUP_PATH}
	pg_ctl stop -m fast > /dev/null 2>&1
	rm -fr ${LOG_DIRECTORY} # because pg_rman restores online serverlogs
	mkdir -p ${LOG_DIRECTORY}
	pg_rman restore -B ${BACKUP_PATH} --quiet;echo $?
	check_log ${LOG_DIRECTORY} > ${TEST_BASE}/${TEST_NAME}-after.out
	diff ${TEST_BASE}/${TEST_NAME}-before.out ${TEST_BASE}/${TEST_NAME}-after.out > /dev/null 2>&1; echo $? # 1 if diff exists. This means that server logs don't be backuped
}
...
# Test for restoring serverlogs
echo '###### RESTORE COMMAND TEST-0013 ######'
echo '###### server logs (in the default log_directory) do not be backuped without "--with-serverlog" ######'
init_backup --default-log_directory
DEFAULT_SRVLOG_PATH=${PGDATA_PATH}/`get_guc_value log_directory`
restore_serverlog_tests "TEST-0013" ${DEFAULT_SRVLOG_PATH}
echo ''

echo '###### RESTORE COMMAND TEST-0014 ######'
echo '###### server logs (in the default log_directory) should be backuped with "--with-serverlog" ######'
init_backup --default-log_directory
DEFAULT_SRVLOG_PATH=${PGDATA_PATH}/`get_guc_value log_directory`
restore_serverlog_tests "TEST-0014" ${DEFAULT_SRVLOG_PATH} "--with-serverlog"
echo ''

echo '###### RESTORE COMMAND TEST-0015 ######'
echo '###### server logs do not be backuped without "--with-serverlog" ######'
init_backup
restore_serverlog_tests "TEST-0015" ${SRVLOG_PATH}
echo ''

echo '###### RESTORE COMMAND TEST-0016 ######'
echo '###### server logs should be backuped with "--with-serverlog" ######'
init_backup
restore_serverlog_tests "TEST-0016" ${SRVLOG_PATH} "--with-serverlog"
echo ''
